...
function TableBodyInner<T extends Record<string, unknown>>({
  ...
  /** Render array fields as chips with a tooltip on the overflow badge */
  ...
  /** Render cell value using column config — type-driven */
  const renderCellValue = (column: ColumnConfig<T>, row: T): React.ReactNode => {
    ...
    // Null / undefined — show dash (TC-047, TC-048 safe fallback)
    if (value == null) {
