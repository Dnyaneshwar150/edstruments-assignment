...
export const DynamicInput: React.FC<DynamicInputProps> = React.memo(({
      ...
      return (
        <Stack spacing={1.5}>
          <TextField
            ...
            label="From"
            value={rangeValues[0] || ''}
            onChange={(e) => onChange([e.target.value, String(rangeValues[1] || '')])}
          ...
          <TextField
            ...
            label="To"
            value={rangeValues[1] || ''}
            onChange={(e) => onChange([String(rangeValues[0] || ''), e.target.value])}
            InputLabelProps={{ shrink: true }}
            fullWidth
          />
        </Stack>
  ...
  // ── Multiple values (is any of, in, not in) ──
  if (operator.valueCount === 'multiple') {
          ...
          >
            {column.options.map(opt => (
              <MenuItem key={opt.value} value={opt.value}>
                <Checkbox checked={selectedValues.includes(opt.value)} size="small" />
    ...
    // Free-text "is any of" — Autocomplete with chips
    ...
    return (
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField {...params} label="Values (press Enter)" placeholder="Add value..." />
        )}
      />
    );
  }

  // ── Single value inputs by type ──
  switch (column.type) {
    case 'text':
      return (
        <TextField
